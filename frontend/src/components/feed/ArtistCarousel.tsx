...
export const ArtistCarousel = ({ onAddClick }: { onAddClick: () => void }) => {
  ...
  return (
    <div className="flex items-center gap-4 py-4 overflow-x-auto snap-x snap-mandatory scroll-smooth [&::-webkit-scrollbar]:h-2 [&::-webkit-scrollbar-track]:bg-slate-100 [&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb]:rounded-full">
      ...
      >
        <div
          className={`w-16 h-16 rounded-full border-2 border-dashed border-slate-300 flex items-center justify-center ${text.muted} group-hover:border-green-500 group-hover:text-green-500 transition-colors`}
        >
          <svg
      ...
      {artists.map((artist) => (
        <div
          key={artist.id}
          className="flex-shrink-0 flex flex-col items-center gap-2 w-16 mt-1 relative snap-center"
        >
          <div className="relative">
            ...
            <button
              ...
              className={`absolute -top-1 -right-1 ${button.icon.size.md} bg-white/90 ${button.icon.base} ${button.icon.color.red} ${button.icon.rounded} ${shadow.sm}`}
              title="Unsubscribe"
            >
              <X size={12} />
            </button>
          ...
          <span
            className={`text-[10px] font-bold ${text.secondary} truncate w-full text-center`}
          >
            {artist.name?.split(" ")[0]}
          </span>
        </div>
